...
- **Constraint B**: Never modify the `104a.mfdb.bejson` manifest without ensuring the `Fields` and `Values` arrays are perfectly aligned. Use `bejson-manager` for structural validation.
...
3. **Execute**: Use surgical edits for registry updates; use the `protocol-update` for cross-skill refactors.
4. **Verify**: Run `analysis-perceptual` to confirm the system's structural integrity post-modification.
